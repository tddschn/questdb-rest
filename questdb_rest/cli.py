...
def get_table_name_add_prefix(p: Path, prefix: str = "", **kwargs) -> str:
    ...
    prefix_str = prefix # Removed default 'import_' here, rely on arg default or presence
...
def simulate_imp(args, file_path, table_name, schema_source):
    ...
    params = {
        "name": table_name, "partitionBy": args.partitionBy, "timestamp": args.timestamp,
        "overwrite": str(args.overwrite).lower() if args.overwrite is not None else None,
        "atomicity": args.atomicity, "delimiter": args.delimiter,
        "forceHeader": str(args.forceHeader).lower() if args.forceHeader is not None else None,
        "skipLev": str(args.skipLev).lower() if args.skipLev is not None else None,
        "fmt": args.fmt, "o3MaxLag": args.o3MaxLag,
    ...
    if args.fmt == "json":
        print(json.dumps({
            "dry_run": True, "operation": "import", "status": "OK (Simulated)",
            "location": table_name, "rowsRejected": 0, "rowsImported": 0,
            "header": bool(args.forceHeader) if args.forceHeader is not None else False,
...
def simulate_exec(args, statement, statement_index, total_statements):
     logger.info(f"[DRY-RUN] Simulating /exec request ({statement_index}/{total_statements}):")
     params = {
         "query": statement, "limit": args.limit,
         ...
         "quoteLargeNum": str(args.quoteLargeNum).lower() if args.quoteLargeNum is not None else None,
     }
     filtered_params = {k: v for k, v in params.items() if v is not None}
     logger.info(f"[DRY-RUN]   Params: {filtered_params}")
     headers = {}
                ...
                if args.name_func:
                    ...
                    if name_func_choice in TABLE_NAME_FUNCTIONS:
                        ...
                        if "prefix" in req_args:
                            func_kwargs["prefix"] = args.name_func_prefix or "" # Pass empty string if None
                        try:
                            table_name = func(file_path, **func_kwargs)
                            logger.info(f"Using name function '{name_func_choice}' -> table name: '{table_name}'")
                        except Exception as e:
                            logger.error(f"Error executing name function '{name_func_choice}' for file '{file_path}': {e}")
                            any_file_failed = True
                            if args.stop_on_error: sys.exit(1)
                            else: continue
                    else:
                        logger.error(f"Internal error: Unknown name function '{name_func_choice}'.")
                        any_file_failed = True
                        if args.stop_on_error: sys.exit(1)
                        else: continue
                else:
                    table_name = get_table_name_from_stem(file_path)
                    logger.info(f"Using default naming (file stem) -> table name: '{table_name}'")

            if not table_name:
                logger.error(f"Could not determine table name for file '{file_path}'. Skipping.")
                any_file_failed = True
                if args.stop_on_error: sys.exit(1)
                else: continue
            ...
            try:
                ...
                try:
                    if args.fmt == "json":
                        ...
                        if response_json.get("status") != "OK":
                             ...
                             logger.warning(f"Import of '{file_path}' failed (JSON status: {response_json.get('status')}).")
                             if "errors" in response_json: # Log column errors if present
                                 logger.warning(f"Column Errors: {response_json['errors']}")

                    else: # Tabular format
                        response_text = response.text
                        # Basic check for tabular failure (less reliable) - might need improvement
                        if "error" in response_text.lower() or response.status_code >= 400:
                             import_failed_this_file = True
                             logger.warning(f"Import of '{file_path}' may have failed (status code: {response.status_code}).")

                except json.JSONDecodeError:
                    import_failed_this_file = True
                    logger.warning(f"File '{file_path}': Received non-JSON response when JSON format was requested.")
                    response_text = response.text # Get text for logging
                    logger.warning(f"Raw response: {response_text[:500]}") # Log first 500 chars
                ...
                if import_failed_this_file:
                    ...
                    if args.stop_on_error:
                        logger.warning("Stopping execution due to import failure (stop-on-error enabled).")
                        sys.exit(1)
                else:
                     logger.info(f"File '{file_path}' processed.")


            except (QuestDBError, OSError, IOError) as e: # Catch client errors and file errors
                logger.warning(f"Processing failed for file '{file_path}': {e}")
                any_file_failed = True
                if args.stop_on_error:
                    logger.warning("Stopping execution due to error (stop-on-error enabled).")
                    sys.exit(1)
                else:
                    logger.warning("Continuing with next file (stop-on-error disabled).")
...
def handle_exec(args, client: QuestDBClient):
    ...
    for i, statement in enumerate(statements):
        ...
        logger.debug(f"Statement: {statement[:100]}{'...' if len(statement) > 100 else ''}")
        ...
        try:
            ...
            if isinstance(response_json, dict) and "error" in response_json:
                logger.error(f"Error executing statement {i + 1}: {response_json['error']}")
            ...
            any_statement_failed = True
            if args.stop_on_error:
                logger.warning("Stopping execution due to API error (stop-on-error enabled).")
                sys.exit(1)
            else:
                logger.warning("Continuing execution (stop-on-error disabled).")
    ...
    try:
        ...
        if args.output_file:
            ...
            try:
                ...
                logger.warning(f"Error writing to output file '{output_file_path}': {e}")
                sys.exit(1)
        else:
            # Write text response directly to stdout
            output_text = response.text # Assumes content fits in memory if not streaming
...
def handle_chk(args, client: QuestDBClient):
    ...
    try:
        ...
        print(json.dumps({"tableName": table_name, "status": "Error", "detail": str(e)}, indent=2))
        sys.exit(1)
    except QuestDBError as e:
...
def main():
    ...
    # Global arguments
    parser.add_argument("-H", "--host", default=DEFAULT_HOST, help=f"QuestDB server host (default: {DEFAULT_HOST}).")
    parser.add_argument("--port", type=int, default=DEFAULT_PORT, help=f"QuestDB REST API port (default: {DEFAULT_PORT}).")
    parser.add_argument("-u", "--user", help="Username for basic authentication.")
    parser.add_argument("-p", "--password", help="Password for basic authentication. If -u is given but -p is not, will prompt securely.")
    parser.add_argument("--timeout", type=int, default=QuestDBClient.DEFAULT_TIMEOUT, help=f"Request timeout in seconds (default: {QuestDBClient.DEFAULT_TIMEOUT}).")
    parser.add_argument("-D", "--debug", action="store_true", help="Enable debug level logging to stderr.")
    parser.add_argument("-R", "--dry-run", action="store_true", help="Simulate API calls without sending them. Logs intended actions.")

    subparsers = parser.add_subparsers(dest="command", required=True, help="Available sub-commands")

    # --- IMP Sub-command ---
    parser_imp = subparsers.add_parser("imp", help="Import data from file(s) using /imp.", formatter_class=argparse.ArgumentDefaultsHelpFormatter, add_help=False)
    parser_imp.add_argument("-h","--help",action="help",default=argparse.SUPPRESS,help="Show this help message and exit.",)
    parser_imp.add_argument("files", nargs="+", help="Path(s) to the data file(s) to import.")
    parser_imp.add_argument("-n", "--name", help="Explicit table name. Overrides --name-func. Applied to ALL files.")
    parser_imp.add_argument("--name-func", choices=list(TABLE_NAME_FUNCTIONS.keys()), help=f"Function to generate table name from filename (ignored if --name set). Available: {', '.join(TABLE_NAME_FUNCTIONS.keys())}", default=None)
    parser_imp.add_argument("--name-func-prefix", help="Prefix string for 'add_prefix' name function.", default="") # Default to empty string
    schema_group = parser_imp.add_mutually_exclusive_group()
    schema_group.add_argument("--schema-file", help='Path to JSON schema file. Applied to ALL files.')
    schema_group.add_argument("-s", "--schema", help='JSON schema string. Applied to ALL files. Use quotes.')
    parser_imp.add_argument("-P","--partitionBy", choices=["NONE", "YEAR", "MONTH", "DAY", "HOUR", "WEEK"], help="Partitioning strategy (if table created).")
    parser_imp.add_argument("-t","--timestamp", help="Designated timestamp column name (if table created).")
    parser_imp.add_argument("-o","--overwrite", action=argparse.BooleanOptionalAction, help="Overwrite existing table data/structure.")
    parser_imp.add_argument("-a","--atomicity", choices=["skipCol", "skipRow", "abort"], default="skipCol", help="Behavior on data errors during import.")
    parser_imp.add_argument("-d","--delimiter", help='Specify CSV delimiter character.')
    parser_imp.add_argument("-F","--forceHeader", action=argparse.BooleanOptionalAction, help="Force treating the first line as a header.")
    parser_imp.add_argument("-S","--skipLev", action=argparse.BooleanOptionalAction, help="Skip Line Extra Values.")
    parser_imp.add_argument("--fmt", choices=["tabular", "json"], default="tabular", help="Format for the response message to stdout.")
    parser_imp.add_argument("-O", "--o3MaxLag", type=int, help="Set O3 max lag (microseconds, if table created).")
    parser_imp.add_argument("-M", "--maxUncommittedRows", type=int, help="Set max uncommitted rows (if table created).")
    parser_imp.add_argument("-c", "--create", action=argparse.BooleanOptionalAction, default=True, help="Automatically create table if it does not exist.")
    parser_imp.add_argument("--stop-on-error", action=argparse.BooleanOptionalAction, default=True, help="Stop execution immediately if importing any file fails.")
    parser_imp.set_defaults(func=handle_imp)

    # --- EXEC Sub-command ---
    parser_exec = subparsers.add_parser("exec", help="Execute SQL statement(s) using /exec (returns JSON).\nReads SQL from --query, --file, or stdin.", formatter_class=argparse.RawTextHelpFormatter, add_help=False)
    parser_exec.add_argument("-h","--help",action="help",default=argparse.SUPPRESS,help="Show this help message and exit.",)
    ...
    query_input_group.add_argument("-f", "--file", help="Path to file containing SQL statements.")
    parser_exec.add_argument("-l", "--limit", help='Limit results (e.g., "10", "10,20"). Applies per statement.')
    parser_exec.add_argument("-C", "--count", action=argparse.BooleanOptionalAction, help="Include row count in response.")
    parser_exec.add_argument("--nm", dest="nm", action=argparse.BooleanOptionalAction, help="Skip metadata in response.")
    parser_exec.add_argument("-T", "--timings", action=argparse.BooleanOptionalAction, help="Include execution timings.")
    parser_exec.add_argument("-E", "--explain", action=argparse.BooleanOptionalAction, help="Include execution plan details.")
    parser_exec.add_argument("-Q", "--quoteLargeNum", action=argparse.BooleanOptionalAction, help="Return LONG numbers as quoted strings.")
    parser_exec.add_argument("--statement-timeout", type=int, help="Query timeout in milliseconds (per statement).")
    parser_exec.add_argument("--stop-on-error", action=argparse.BooleanOptionalAction, default=True, help="Stop execution if any SQL statement fails.")
    parser_exec.set_defaults(func=handle_exec)

    # --- EXP Sub-command ---
    parser_exp = subparsers.add_parser("exp", help="Export data using /exp (returns CSV to stdout or file).", formatter_class=argparse.ArgumentDefaultsHelpFormatter, add_help=False)
    parser_exp.add_argument("-h","--help",action="help",default=argparse.SUPPRESS,help="Show this help message and exit.",)
    parser_exp.add_argument("query", help="SQL query for data export.")
    parser_exp.add_argument("-l", "--limit", help='Limit results (e.g., "10", "10,20", "-20").')
    parser_exp.add_argument("--nm", dest="nm", action=argparse.BooleanOptionalAction, help="Skip header row in CSV output.")
    parser_exp.add_argument("-o", "--output-file", help="Path to save exported CSV data (default: stdout).")
    parser_exp.set_defaults(func=handle_exp)

    # --- CHK Sub-command ---
    parser_chk = subparsers.add_parser("chk", help="Check if a table exists using /chk (returns JSON).", formatter_class=argparse.ArgumentDefaultsHelpFormatter, add_help=False)
    parser_chk.add_argument("-h","--help",action="help",default=argparse.SUPPRESS,help="Show this help message and exit.",)
    ...
    try:
        ...
        if args.user and not args.password and not args.dry_run: # Don't prompt in dry-run
            try:
        ...
        if args.command == "imp":
             if args.name_func == "add_prefix" and not args.name_func_prefix:
                 logger.debug("Using default empty prefix for 'add_prefix' name function as --name-func-prefix was not provided.")
             elif args.name_func and args.name:
                 logger.warning("Both --name and --name-func provided. Explicit --name will be used.")
