...
class QuestDBClient:
    ...
    def _request(
        ...
        files: Optional[Dict[str, Tuple[Optional[str], Union[bytes, IO[bytes]], Optional[str]]]] = None,
        ...
        try:
            ...
            try:
                ...
                if isinstance(error_data, dict):
                    ...
                    elif "status" in error_data and error_data["status"] != "OK": # /imp error format
                         err_msg = f"HTTP {status_code}: Import failed - {error_data['status']}"
                    elif "status" in error_data and "Exists" in error_data["status"]: # /chk error format
                ...
                logger.warning(f"Response Body: {json.dumps(error_data)}") # Log full JSON error
            except json.JSONDecodeError:
                logger.warning(f"QuestDB API Error: {err_msg} (Non-JSON response)")
                logger.warning(f"Raw Response Body: {e.response.text}")

            raise QuestDBAPIError(err_msg, status_code=status_code, response_data=error_data) from e
    ...
    def imp(
        ...
        if not effective_table_name:
             if data_file_path:
                 ...
                 effective_table_name = data_file_path.split('/')[-1].split('\\')[-1].rsplit('.', 1)[0]
             elif data_file_name:
        ...
        params = {
            ...
            "atomicity": atomicity,
            "delimiter": delimiter,
            "forceHeader": str(force_header).lower() if force_header is not None else None,
        ...
        files_for_request: Dict[str, Tuple[Optional[str], Union[bytes, IO[bytes]], Optional[str]]] = {}
        data_f = None
        schema_f = None
        ...
        try:
            ...
            if data_file_path:
                ...
                files_for_request["data"] = (actual_filename, data_f, "application/octet-stream")
            else: # data_file_obj must be set
                files_for_request["data"] = (data_file_name, data_file_obj, "application/octet-stream") # type: ignore

            # Prepare Schema Part (if provided)
            schema_sources = sum(1 for src in [schema_json_str, schema_file_path, schema_file_obj] if src is not None)
            if schema_sources > 1:
                raise ValueError("Provide only one of schema_json_str, schema_file_path, or schema_file_obj.")

            schema_content: Optional[Union[bytes, IO[bytes]]] = None
            if schema_json_str:
                schema_content = schema_json_str.encode('utf-8')
                files_for_request["schema"] = ("schema.json", schema_content, "application/json")
            elif schema_file_path:
                schema_f = open(schema_file_path, "rb")
                files_for_request["schema"] = ("schema.json", schema_f, "application/json")
            elif schema_file_obj:
                 files_for_request["schema"] = ("schema.json", schema_file_obj, "application/json")
    ...
    def exec(
        ...
        params = {
            ...
            "quoteLargeNum": str(quote_large_num).lower() if quote_large_num is not None else None,
        }
        headers = {}
...
if __name__ == '__main__':
    ...
    client = QuestDBClient(host=TEST_HOST, port=TEST_PORT, user=TEST_USER, password=TEST_PASSWORD)
    ...
    try:
        ...
        try:
             print(f"Import Response Body (JSON): {json.dumps(imp_response.json(), indent=2)}")
        except json.JSONDecodeError:
